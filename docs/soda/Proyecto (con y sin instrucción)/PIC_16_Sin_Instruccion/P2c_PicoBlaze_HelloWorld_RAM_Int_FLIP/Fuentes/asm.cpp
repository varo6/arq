...
char *flip_id = "011111";                                          //Codigo de operacion, 5 bits más significativos de los 16 que forman las instrucción
																   //Solamente admite 32 operaciones (como maximo), actualmente tiene 30 con la flip que acabamos de poner
															       //Los codigos de operacion noo se pueden repetir

char *mix_id = "101001";						
																	// Identificador para la nueva instruccion MIX, lo hemos hecho distinto al resto de manera que sea facilmente reconocible (sobre todo para comprobaciones)/* input/output group */
...
#define MAX_LINE_COUNT 65536 /* max 10000 lines allowed */            // Máximo numero de lineas del programa, incluye lineas con comentarios.
#define PROGRAM_COUNT 65536	/* total program word */                // Maximo numero de instrucciones en el programa
        
/* increase instruction_count for added new instruction */
#define instruction_count 31 /* total instruction set */             // Numero maximo que tenemos en el codigo para usar

#define CONSTANT_COUNT 100	/* max 100 constant can be declared */
#define REG_COUNT 16			/* max 16 namereg can be declared */     // Maximo Numero de registros 
...
char *instruction_set[] = {
	...
	"LOAD",		/* 3 */
	"AND",		/* 4 */
	"OR",		/* 5 */			//Se van recorriendo con la "j" del bucle for de abajo (linea 370)
	"XOR",		/* 6 */
	"ADD",		/* 7 */
	"ADDCY",	/* 8 */
	"SUB",		/* 9 */
	...
	"NAMEREG",	/* 27 */
	"ADDRESS",	/* 28 */
	"FLIP", 	/* 29 */ /* added new instruction */					///Hace un mirror de los bits en sus posiciones en espejo. Por lo que nos convierte la letra que pulsemos de teclado en otro caracter ASCII (si pulsas 'a' ya no sale 'a')
	"MIX"};		/* 30 */ /* added new instruction */			// Instruccion nueva sugerida por el profesor en las practicas, para hacer el cambio de los 8 bits primeros con los 8 bits segundos
																// Lo implementamos para poder hacer con mayor facilidad el programa original que nos devuelve el mensaje de la RAM, la tecla presionada, y un valor numerico 'aleatorio' de tiempo
																// Al introducir esta nueva instruccion se facilita en gran medida este objetivo que tenemos con nuestro proyecto
int error = 0;
/*====================================== */
...
/*====================================== */
/* Only S0 - S15 are valid */                                               // Comprueba la sintaxis de los registros.
...
void insert_instruction(char *s, int p)                                    // crea codigo máquina: campo de instrucciones
{
	...
	program_word[p] = program_word[p] | (n << 20);  // Esto es debido a que la nueva instruccion tiene 6 bits y hay que despalazarla 20 sitios para que empiece en el 26
}
...
void test_instructions(void)
{
	...
	for(i = 0; i < line_count; i++){                                      // para todas las lineas del programa
		if(op[i].instruction != NULL){                                    // si la linea contiene codigo de operación comprueba si conincide con las definidas en el juego de instrucciones
			...
			/* add (j > 28) for FLIP or MIX instruction, - added new instruction */
		...
		if(op[i].instruction != NULL){                                                   // si la linea contiene codigo de operación comprueba si conincide con las definidas en el juego de instrucciones
			...
			switch (j)                                                                   // comprueba a que clase de codigo de operación pertenece
			{
				...
				case 10: /* SUBCY */
					...
					if(j == 8){ kptr = addcy_k_to_x_id; sptr = addcy_y_to_x_id;}				///si queremos añadir una operación nueva solo temdriamos que añadir una nueva j
					...
					else if((reg_n = register_number(op[i].op1)) != -1)							///Traduce primer operando nombre, registro
					...
					if((reg_n = find_constant(op[i].op2)) != -1){                                 // comprueba si el 2do operando es un namereg, un registro, una constante o un hexadecimal, sino error.
						...
						insert_instruction(kptr,op[i].address);									///Traduce el segundo registro o nombre de hex a binario
				...
				case 30: /* MIX */ /* added new instruction */	
					if(j == 29) insert_instruction(flip_id, op[i].address);                          // inserta codigo máquina del codigo de operacion.
...
void write_vhd(void)
{
	...
	*ptr = '\0';
	...
	fprintf(ffp,"\tconstant ROM_WIDTH: INTEGER:= 26;\n");			 //Cambiamos el size que tendra acada elemento de la ROM
